...
{
	exports[command] = getCommander(command);
}

/**
 * Get a function that runs a command.
 */
...
{
	exports.stats(function(error, result)
	{
		testing.check(error, 'Error in stats', callback);
		testing.assert(result.countProperties() > 5, 'Too few stats', callback);
		exports.version(function(error, result)
		{
			testing.check(error, 'Could not version', callback);
			testing.assert(result.startsWith('nodecached'), 'Invalid version ' + result, callback);
			exports.flush(function(error, result)
			{
				testing.check(error, 'Could not flush', callback);
				testing.assert(result, 'Did not flush', callback);
				testing.success(callback);
			});
		});
	});
