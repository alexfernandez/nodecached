...
const ERROR_BUFFER = new Buffer('ERROR\r\n');
...
exports.Server = function(options, callback)
{
	...
	function init()
	{
		...
		{
			const message = 'Could not start server on port ' + options.port + ': ' + error;
			if (callback)
			{
 ...
 */
const Connection = function(options, log)
{
	// self-reference
	const self = this;
	...
	function readData(data)
	{
		...
		// end optimization
		let result = parser.readLine(line);
		if (rest)
		{
			if (result)
			{
...
function testStart(callback)
{
	...
	const server = exports.start(options, function(error)
	{
