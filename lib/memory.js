...
exports.Cache = function(options)
{
	...
	self.incr = function(key, value)
	{
		...
		var original = parseFloat(record.value);
...
function testIncrDecr(callback)
{
	var key = 'test#' + token.create();
	var cache = new exports.Cache();
	testing.assert(cache.setRecord(key, '10'), 'Should set record', callback);
	...
	testing.assert(cache.setRecord(key, 'aaa'), 'Should set record again', callback);
	try
	{
		cache.incr(key, 5);
