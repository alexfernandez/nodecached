 ...
 */
exports.Record = function(flags, expirationSeconds, value)
{
	// self-reference
	var self = this;
...
{
	var record = new exports.Record(null, 1, 'b');
	testing.assert(record.isValid(), 'Should be valid', callback);
	record = new exports.Record(null, -1, 'b');
...
function testGetRecord(callback)
{
	...
	var value = {a: 'b'};
	testing.assertEquals(exports.getRecord(key), null, 'Could get record before setting it', callback);
	var record = new exports.Record(0, 0, value);
	testing.assert(exports.setRecord(key, record), 'Could not set record', callback);
	var result = exports.getRecord(key);
	testing.assert(result, 'Could not get record', callback);
	testing.assertEquals(result.value, value, 'Got different record', callback);
	testing.assert(exports.deleteRecord(key), 'Could not delete record', callback);
	testing.assertEquals(exports.getRecord(key), null, 'Could get record after deleting it', callback);
	testing.success(callback);
}
...
{
	cache[key] = new exports.Record(0, lifetime, value);
	if (callback)
	{
		callback(null, true);
	}
