...
exports.Cache = function(options)
{
	 ...
	 *	- params: for storage.
	 */
	self.setItem = function(key, value, params)
	{
		var lifetime = 0;
		if (params.expirationAbsolute)
		{
			lifetime = (Date.now() - params.expirationAbsolute.getTime()) / 1000;
		}
...
function getDefaultCache()
{
	...
	{
		log.info('Creating default cache');
...
exports.get = function(key, callback)
{
	...
	var record = getDefaultCache().getRecord(key);
	if (!record)
	{
		log.info('no record');
...
{
	var record = new Record(0, lifetime, value);
	getDefaultCache().setRecord(record);
	if (callback)
	{
		callback(null, true);
	}
