...
function testGetSet(callback)
{
	...
	testing.assertEquals(cache.get(key).value, appended, 'Invalid appended', callback);
	testing.assert(cache.prepend(key, 'bb'), 'Could not preppend', callback);
	testing.assertEquals(cache.get(key).value, 'bb' + appended, 'Invalid prepended', callback);
