...
class Client {
	/**
	 * Create a client.
	 * For async usage, call await client.connect().
	 * Params:
	...
	getDistributor(name) {
		const self = this
		return function() {
			const connection = self.pickRandomConnection();
			const commandFunction = connection[name];
			if (!commandFunction)
			{
				self.log.error('Connection missing the %s function', name);
				return;
			}
			const result = commandFunction.apply(connection, arguments);
			console.log(`r ${result} name ${commandFunction.name}`)
			return result
		};
	}
...
class ServerConnection {
	...
	getAutomaticCommander(command, syntax) {
		...
		return function() {
			...
			self.write(output.join(' ') + '\r\n');
			const mapped = syntaxes.remap(command);
			const parser = self['parse' + mapped.capitalize()];
			if (callback) {
				console.log(`callback`)
				self.pending = self.getPending(mapped, callback, parser && parser.bind(self));
				return
			}
			console.log('promise')
			return new Promise((resolve, reject) => {
...
async function testPromises()
{
	...
	const client = new Client(location, options)
	await client.connect()
	let result = await client.set(key, value, 10)
	testing.assertEquals(result, true, 'Invalid set result');
	result = await client.get(key)
	testing.assertEquals(result, value, 'Invalid get result');
	result = await client.delete(key)
	testing.assertEquals(result, true, 'Invalid delete result');
	result = await client.get(key)
	testing.assertEquals(result, null, 'Invalid get deleted result')
	await client.end()
	await server.stop();
}
