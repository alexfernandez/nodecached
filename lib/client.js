...
var ServerConnection = function(location, options, callback)
{
	...
	function init()
	{
		...
		for (var command in syntaxes.commandMap)
		{
			...
			self[command] = getAutomaticCommander(command, syntax);
		}
	}
	 ...
	 */
	function getAutomaticCommander(command, syntax)
	{
		return function()
		{
			var callback;
			var output = [syntax.command || command];
			...
			write(output.join(' ') + '\r\n');
			var mapped = syntaxes.remap(command);
			var parser = self['parse' + mapped.capitalize()];
			pending = getPending(mapped, callback, parser);
		};
	}
	 ...
	 */
	self.parseGet = function(message, callback)
	{
	 ...
	 */
	self.parseIncr = function(message, callback)
	{
	 ...
	 */
	self.parseStats = function(message, callback)
	{
		var stats = {};
		var lines = message.split('\r\n');
	 ...
	 */
	self.parseVersion = function(message, callback)
	{
