...
var ServerConnection = function(location, options, callback)
{
	...
	{
		pending = getPending('get', callback, parseGet);
		write('get ' + key + '\r\n');
	};

	/**
	 * Parse the output of a get.
	 */
	function parseGet(message, callback)
	{
		var parsed = parseGetMessage(message);
		if (!parsed)
		{
	...
	/**
	 * Parse the message that results from a get, return an object.
	 */
	 ...
	 */
	self.append = function(key, value, callback)
	{
		set(key, value, 0, 'append', callback);
	};

	/**
	 * Prepend to an existing value.
	 */
	self.prepend = function(key, value, callback)
	{
		set(key, value, 0, 'prepend', callback);
	};

	function set(key, value, exptime, command, callback)
	{
		pending = getPending('set', callback);
		var message = value;
	...
	{
		pending = getPending('delete', callback);
		write('delete ' + key + '\r\n');
	};
	...
	{
		incrDecr('incr', key, value, callback);
	};
	...
	{
		incrDecr('decr', key, value, callback);
	};

	function incrDecr(command, key, value, callback)
	{
		pending = getPending(command, callback, parseIncr);
		write(command + ' ' + key + ' ' + value + '\r\n');
	}

	/**
	 * Parse the output of an incr.
	 */
	function parseIncr(message, callback)
	{
	...
	{
		pending = getPending('touch', callback);
	...
	{
		pending = getPending('stats', callback, parseStats);
		write('stats\r\n');
	};

	/**
	 * Parse the output of stats.
	 */
	function parseStats(message, callback)
	{
		var stats = {};
		var lines = message.split('\r\n');
	...
	{
		pending = getPending('flush', callback);
		write('flush_all\r\n');
	};
	...
	{
		pending = getPending('version', callback, parseVersion);
		write('version\r\n');
	};

	/**
	 * Parse the output of the version command.
	 */
	function parseVersion(message, callback)
	{
	 ...
	 */
	function getPending(command, callback, handler)
	{
		if (options.noResponse)
		{
			return callback;
		...
		return function(error, result)
		{
			...
			{
				log.debug('Checking %s: %s', key, output[key]);
				if (output[key] == result)
				{
					...
					{
						log.error('Could not parse %s', key);
			...
			if (!handler)
			{
				log.error('Unrecognized result %s for command %s', result, command);
	...
	/**
	 * Locate the given output syntax for a command.
	 */
	function getOutput(command)
	{
		...
		log.debug('Output: %j', syntax.output);
