...
exports.Parser = function(options)
{
	...
	function readCommand(line)
	{
		...
		var options = {};
		for (var key in syntax.protocol)
		{
			var type = syntax.protocol[key];
			if (words.length === 0 && !type.isOptional)
			{
	...
	function readData(line)
	{
		...
		self.data += line;
		self.options.value = self.data;
		var result = interpreter.interpret(self.command, self.options);
		reset();
		return result;
	}
...
function testOtherLines(callback)
{
	...
	testing.assertEquals(parser.readLine('flush_all'), 'OK', 'Should flush', callback);
	testing.assertEquals(parser.readLine('verbosity 5'), 'OK', 'Should set verbosity', callback);
	testing.assertEquals(parser.readLine('quit'), 'quit', 'Could not quit', callback);
	testing.success(callback);
}
