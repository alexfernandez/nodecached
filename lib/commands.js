...
exports.Interpreter = function(options)
{
	...
	self.interpret = function(command, options, data)
	{
		...
		var result = commandFunction(options, data);
	...
	{
		return set(options, data, cache.set);
	};
	...
	{
		return set(options, data, cache.add);
	};
	...
	{
		return set(options, data, cache.replace);
	};
	...
	{
		options.value = data;
		return fn(options.key, data, options.exptime, options.flags);
	}
...
function testInterpretCommands(callback)
{
	...
	var replaceResult = interpreter.interpret('replace', options, value);
